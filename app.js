    ...
    if (req.params.role === 'student') {
        ...
        Group.findOne({
            ...
            if(err){
                ...
                res.json({auth:true,response:data,imgURL:req.oidc.user.picture});
            }
        });
    } else {
        res.json({auth:false});
...
app.post('/api/enrollcourse',requiresAuth(),(req, res) => {
    User_Student.findOne({authID : req.oidc.user.sub},(err,Userdata)=>{
            Courses.findOne({_id : req.body.course_id},'_id subscription_plan course_title',(err,courseData)=>{
                ...
                if((!err && authReq())){
                    User_Student.updateOne({authID : req.oidc.user.sub},{$push :{
                        enrolled_courses :{
...
app.get('/api/explore', (req, res) => {
    Courses.find({status : true},'_id course_title course_features subscription_plan short_description' ,(err, data) => {
        if(!err)
        res.json({
            data: data,auth : req.oidc.isAuthenticated()
        });
        else {
            console.log(err);
            res.sendStatus(500);
...
app.get('/api/explore/:courseID',(req,res)=>{
    Courses.findOne({_id : req.params.courseID, status  : true},'_id course_title course_features subscription_plan course_img course_description modules.title modules.content', (err,resData)=>{
        if(!err)
        res.json({data : resData,response : true,auth : req.oidc.isAuthenticated()});
        else 
...
app.get('/api/mylearning/:courseID',requiresAuth(),(req,res)=>{
    User_Student.findOne({authID: req.oidc.user.sub},(err,data)=>{
        function getCourse(){
        ...
        if(getCourse()){
            Courses.findOne({_id : req.params.courseID,status : true },(cerr,courseData)=>{
                if(!cerr){
                    res.json({course_data : courseData,user_data : data,access : true})
                }
            });
        } else {
...
app.post('/api/mylearning/:courseID/:moduleNum',requiresAuth(),(req,res)=>{
    User_Student.updateOne({authID : req.oidc.user.sub,"enrolled_courses.course_id" : req.params.courseID},{"enrolled_courses.$.course_completion": req.params.moduleNum},(err)=>{
        if(!err)
            res.sendStatus(200);
        else 
            console.log(err);
...
app.get('/api/verify',(req,res)=>{
    if(req.oidc.isAuthenticated()){
        User.findOne({authID:req.oidc.user.sub},(err,data)=>{
            if(err==null){
                res.json({
                    auth : true,
                    imgURL :  req.oidc.user.picture,
...
app.get('/api/profile', requiresAuth(), (req, res) => {
    ...
    if (req.query.role === "mentor") {
        ...
        User_Student.findOne({
            ...
            res.redirect('../dashboard/student/courses');
        });
        ...
        Group.findOne({
            ...
            res.redirect('../dashboard/parent/profile');
        });
    } else {
        res.sendStatus(500);
        ...
        User_Student.findOne({authID : req.oidc.user.sub},(err,data)=>{
            if(!err){
                razorInstance.subscriptions.create({
                    ...
                    if(!subErr) {
                        res.render('paymentPage',{subscription_id : subData.id,key : process.env.RAZOR_KEY_ID,role:"student",planID : req.query.plan, user: data});
                    }
                });
            } else {
                res.sendStatus(500);
...
async function* loadYieldResponse(){
    ...
    yield await User_Student.find({}).sort({date:-1});
    yield await Courses.find({}).count();
    yield await Mentor.find({},{},{sort:{created_at:-1}});
    yield await User_Student.find({subscribed_plan: 'free'}).count();
...
app.get('/admin/get/mentor',(req,res)=>{
    Mentor.find({status:true},"authID email",(err,data)=>{
        if(!err){
...
app.post('/admin/assignMentor', (req, res) => {
    ...
    User_Student.updateOne({
        ...
        if(err == null){
            Mentor.updateOne({authID:req.body.mentor_authID},{
                $push:{
                    students:{
                ...
                if(Menerr == null){
                    Messenger.findOne({_student_id:req.body.student_authID,_mentor_id : req.body.mentor_authID},(Meserr,MesData)=>{
...
app.post('/admin/assigncourse',(req,res)=>{
    Mentor.updateOne({authID : req.body.authID},{$push :{
        assigned_courses : {
...
app.post('/admin/post/module', async (req, res) => {
    ...
    Upload.create({mux_upload_id:newUpload.id,type : "course"},(err,uploadData)=>{
        if (!err){
            Courses.updateOne({
                ...
                if (courerr){
                    ...
                    res.json({res:true,upload_url : newUpload.url});
                }    
            });
        } else {
            res.json({res:false});
...
app.post('/admin/post/access/:query',(req,res)=>{
    if (req.params.query === 'course') {
        Courses.updateOne({_id : req.body.course_id},{status : req.body.status},(err)=>{
            if(!err)
                res.sendStatus(200);
            else
                console.log(err);
        ...
        Courses.updateOne({_id : req.body.course_id, "modules._id" : req.body.module_id},{"modules.$.modules_status" : req.body.status},(err)=>{
            if(!err)
                res.sendStatus(200);
            else
                console.log(err);
        ...
        Courses.updateOne({_id : req.body.course_id , "modules._id" : req.body.module_id},{"modules.$" : {$push : {
            notes : {
                notes_title: req.body.notes_title,
                notes_link : req.body.notes_link
            }}
...
app.post('/admin/archives/access',(req,res)=>{
    try {
        Archive.updateOne({_id : req.body.archive_id},{archive_status : req.body.archive_status},(err)=>{
            if(!err){
                res.json({res:true});
            } else {
                res.json({res : false});
...
app.get('/admin/archives/get/:id',(req,res)=>{
    ...
    .exec((err,data)=>{
        if(err){
            ...
            res.render('archiveView',{data:data});
    });
...
app.post('/admin/archives/module', async (req,res)=>{
    ...
    Upload.create({mux_upload_id:newUpload.id,type : "archive"},(err,uploadData)=>{
        if(!err){
            Archive.updateOne({_id : req.body.archive_id},{$push : {
                modules : {
